...
| LIN | Controller + fixed schedule | 6-bit frame identifier/PID、`0..=8` data bytes、classic/enhanced checksum、publisher/subscriber 和 slot 固定 | 无响应/checksum/schedule miss 可见；只用可测时序且可维护的批准适配器 | LIN SubDevice 仿真、用普通 UART 猜测 break/schedule、动态 schedule |
...
- Aurora 不硬编码其 `0.x` 公共类型。Adapter 负责 state、PDI、WKC/DC、错误和 identity 转换；业务模块
  不得直接依赖 EtherCrab。
- 上游策略为 manifest 跟踪官方 `main`、锁文件固定审核 commit。官方变更不会自动进入同一 Aurora
  commit；显式同步必须重新生成 SBOM 并执行仿真/硬件回归。
...
- SocketCAN interface identity 包含 stable device path、driver/firmware、controller clock、bit timing、
  transceiver capability 和 termination evidence；interface up 不等于工程健康。
...
- LIN adapter 必须原生提供可测 break/sync/PID/checksum/schedule 能力。普通 UART 加 sleep 的实现不能
  通过 capability。schedule 只能来自签名静态表，运行期不能插入任意 frame。
